...
### Fixed
- **Stray `sleep` process leaked on every hook event** — `kill_fade` only
  killed the fade-out subshell, orphaning its child `sleep` (reparented to
  launchd) for the full `FADE_TIMEOUT`. Now kills the child first via
  `pkill -P`, then the subshell.
